...
def add_to_slip(event_id, market, selection, odds):
    ...
    st.session_state.betslip.append(
        {"event_id": event_id, "market": market,
         "selection": selection, "odds": odds, "stake": 10.0}
    )
...
def render_event_card(ev):
    """Build event card HTML as a single string to avoid truncation."""
    ...
    if is_live and "score" in ev:
        ...
        if isinstance(s["home"], int):
            score_html = (
                f'<span class="score-box">{s["home"]} — {s["away"]}</span>'
            )
        else:
            score_html = (
                f'<span class="score-box">{s["home"]} | {s["away"]}</span>'
    ...
    if is_live:
        time_html = (
            f'<span style="color:#ff9800;font-size:12px">'
            f'⏱ {ev.get("minute", "")}</span>'
        )
    else:
        time_html = (
            f'<span style="color:#888;font-size:12px">'
            f'🕐 {ev["date"]} {ev["time"]}</span>'
        )
    ...
    card = (
        ...
        f'<span class="league-tag">{ev["league"]}</span>'
        f'<span>{live_html} {time_html}</span>'
        '</div>'
        '<div style="display:flex;justify-content:space-between;'
        'align-items:center">'
        f'<span class="team-name">{ev["home"]}</span>'
        f'{score_html}'
        f'<span class="team-name">{ev["away"]}</span>'
        '</div>'
        '</div>'
    )
    return card
...
with st.sidebar:
    ...
    if st.button("🔄 Refresh Odds"):
        st.session_state.events = fluctuate_odds(st.session_state.events)
        st.rerun()
    if st.button("🔃 New Events"):
        st.session_state.events = generate_events()
        st.rerun()
...
with main_col:
        ...
        if not events:
            ...
            for sport in sports_in_view:
                ...
                for ev in sport_events:
                    st.markdown(
                        render_event_card(ev), unsafe_allow_html=True
                    )
                    ...
                    for col, label, selection, odd in cols_map:
                        with col:
                            ...
                            prefix = "✓ " if in_slip else ""
                            btn_text = f"{prefix}{label} | {odd}"
                            if st.button(
                                btn_text,
                                key=f"{ev['id']}_{label}",
                ...
                if st.button("🎲 Settle All (Random)", use_container_width=True):
                    for i, bet in enumerate(st.session_state.history):
                        if bet["status"] == "Pending":
                            ...
                            st.session_state.history[i]["status"] = (
    ...
    with tabs[2]:
        if not st.session_state.history:
            ...
            c2.metric(
                "Won", len(won),
                delta=f"+HK${won['return'].sum():.0f}" if len(won) else None,
            )
            c3.metric(
                "Lost", len(lost),
                delta=f"-HK${lost['stake'].sum():.0f}" if len(lost) else None,
            )
            ...
            if len(won) + len(lost) > 0:
                ...
                profit = won["return"].sum() - settled["stake"].sum()
    ...
    if not st.session_state.betslip:
        ...
        for i, bet in enumerate(st.session_state.betslip):
            with st.container():
                ...
                st.markdown(
                    f"<span style='color:#4fc3f7'>Odds: {bet['odds']}</span>",
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    f"<span style='color:#4caf50'>Return: HK${ret:.2f}</span>",
                    unsafe_allow_html=True,
                )
                if st.button(
                    "✕ Remove",
                    key=f"rm_{bet['event_id']}",
                    use_container_width=True,
                ):
